...
class phpmathpublisher {
    ...
    public function __construct(){
        $this->dirfonts = dirname(__FILE__)."/phpmathpublisher/fonts";
        $this->dirimg   = dirname(__FILE__)."/phpmathpublisher/img";
    }
    ...
    function affiche_symbol($texte, $haut) {
        ...
        case '_dintegrale':
        case '_tintegrale':
            if (isset($this->fontesmath[$texte])) $font = $this->dirfonts."/".$this->fontesmath[$texte].".ttf";
            elseif (est_nombre($texte)) $font = $this->dirfonts."/cmr10.ttf";
            else $font = $this->dirfonts."/cmmi10.ttf";
            $t=6;
            if (isset($this->symboles[$texte])) $texte = $this->symboles[$texte];
            do {
                $tmp_dim = imagettfbbox($t, 0, $font , $texte);
                $t+=1;
            }
    ...
    protected function affiche_math($texte, $taille) {
        ...
        $texte=stripslashes($texte);
        if (isset($this->fontesmath[$texte])) $font = $this->dirfonts."/".$this->fontesmath[$texte].".ttf";
        elseif (ereg("[a-zA-Z]", $texte)) $font = $this->dirfonts."/FreeSerifItalic.ttf";
        else $font = $this->dirfonts."/FreeSerif.ttf";
        if (isset($this->symboles[$texte])) $texte = $this->symboles[$texte];
        $htexte = 'dg'.$texte;
        $hdim = imagettfbbox($taille, 0, $font, $htexte);
        $wdim = imagettfbbox($taille, 0, $font, $texte);
...
class expression_texte extends  expression {
     ...
     * @param unknown $taille
     */
    protected function dessine($taille) {
