...
class PMP_expression_math extends  PMP_expression {
    ...
    protected function dessine_delimiteur($taille) {
        ...
        $baseexp=$this->noeuds[2]->base_verticale;
        $hauteurexp=imagesy($imgexp);
        if ($this->noeuds[1]->texte=="&$") $imggauche=$this->parenthese($hauteurexp, $this->noeuds[1]->noeuds[0]->texte);
        else $imggauche=$this->parenthese($hauteurexp, $this->noeuds[1]->texte);
        $basegauche=imagesy($imggauche)/2;
        if ($this->noeuds[3]->texte=="&$") $imgdroit=$this->parenthese($hauteurexp, $this->noeuds[3]->noeuds[0]->texte);
        else $imgdroit=$this->parenthese($hauteurexp, $this->noeuds[3]->texte);
